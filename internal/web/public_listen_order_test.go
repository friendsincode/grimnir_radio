...
func TestListenOrdersStationsByPlatformStationOrderWhenPresent(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	if err != nil {
		t.Fatalf("open db: %v", err)
	}
	...
	// Three public stations with mixed sort/name; we will override ordering via platform stationOrder.
	sA := models.Station{ID: "sA", Name: "Zulu", Active: true, Public: true, Approved: true, SortOrder: 2}
	sB := models.Station{ID: "sB", Name: "Alpha", Active: true, Public: true, Approved: true, SortOrder: 1}
	sC := models.Station{ID: "sC", Name: "Bravo", Active: true, Public: true, Approved: true, SortOrder: 1}
	if err := db.Create(&sA).Error; err != nil {
	...
	// Create a platform landing page config that forces a specific order.
	lp := models.LandingPage{
		ID:        "lp1",
		StationID: nil,
		Theme:     "default",
		PublishedConfig: map[string]any{
			"content": map[string]any{
				"stationOrder": []string{sC.ID, sA.ID, sB.ID},
			},
		},
	}
	if err := db.Create(&lp).Error; err != nil {
		t.Fatalf("create platform landing page: %v", err)
	}
	...
	if !(idxC < idxA && idxA < idxB) {
		t.Fatalf("unexpected platform order: Bravo=%d Zulu=%d Alpha=%d", idxC, idxA, idxB)
	}
}
