...
func TestAutoRecordHandler_StartsRecordingOnEvent(t *testing.T) {
	...
	db.Create(&models.Station{ID: dbtest.UUID("st1"), OwnerID: dbtest.UUID("owner"), Name: "Station One"})
	...
	bus.Publish(events.EventRecordingAutoStart, events.Payload{
		"station_id": dbtest.UUID("st1"), "mount_id": dbtest.UUID("m1"), "user_id": dbtest.UUID("u1"), "username": "dj-nova",
	})
...
func TestHandleAutoRecord_MissingFields_NoOp(t *testing.T) {
	...
	h.handleAutoRecord(bg(), events.Payload{"station_id": dbtest.UUID("st1"), "mount_id": dbtest.UUID("m1")})
