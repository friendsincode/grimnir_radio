...
func TestMetadataTracker_AddsChapterOnNowPlaying(t *testing.T) {
	...
	bus.Publish(events.EventNowPlaying, events.Payload{
		"station_id": "st1", "title": "Song A", "artist": "Artist A",
	})

	deadline := time.Now().Add(2 * time.Second)
	for {
		var n int64
		db.Model(&models.RecordingChapter{}).Where("recording_id = ?", "r1").Count(&n)
		if n == 1 {
			break
		}
...
func TestHandleNowPlaying_NoActiveRecording_NoOp(t *testing.T) {
	...
	mt.handleNowPlaying(bg(), events.Payload{"station_id": "st1", "title": "X", "artist": "Y"})
...
func TestHandleNowPlaying_DedupsIdenticalMetadata(t *testing.T) {
	...
	meta := events.Payload{"station_id": "st1", "title": "Same", "artist": "Same"}
