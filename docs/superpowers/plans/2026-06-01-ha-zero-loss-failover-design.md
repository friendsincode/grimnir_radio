...
- **One listener-facing VIP** held by keepalived, floats between the two HA nodes. This is what `rlmradio.xyz` DNS resolves to (or the regional CNAME does).
...
The current `192.168.195.1` edge VPS reverse-proxies `rlmradio.xyz` to `.11:8081`. This pattern stays. For HA:

- Edge VPS keeps terminating TLS for `rlmradio.xyz`.
...
{
  "streams": [
    {"url": "https://rlmradio.xyz/main/hq", "format": "mp3", "bitrate_kbps": 128, "label": "HQ"},
    {"url": "https://rlmradio.xyz/main/lq", "format": "mp3", "bitrate_kbps": 64,  "label": "LQ"}
  ]
}
```
...
Approach 4 = stand up the complete new HA stack in parallel with current single-host prod (`192.168.195.11` keeps serving listeners throughout). One cutover at the end via the edge VPS reverse-proxy upstream swap. Old stack stays warm for 2–4 weeks as rollback target.
...
6. **Swap edge VPS reverse-proxy upstream** from `192.168.195.11:8081` to the new regional listener VIP. nginx config reload (atomic; existing TCP connections to the edge VPS keep their backend until they reconnect; new connections hit the new stack).
