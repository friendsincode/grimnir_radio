...
R2 is the chosen object-store backend per `docs/superpowers/plans/2026-06-05-object-storage-decision.md`; the backup buckets are separate from the media bucket.

## Scope split

- **Chunk 1** (operational): provision R2 buckets, configure pgbackrest on the Postgres host, verify a full backup completes
...
### Task 1.1: Provision R2 buckets

- [ ] Create `grimnir-backup-<region>` in the same region as `grimnir-media-<region>`
- [ ] Create `grimnir-backup-<region>-dr` in a different region for disaster recovery
- [ ] Generate R2 API token scoped to the two backup buckets only (`Object Read & Write`)
- [ ] Note: account ID, access key, secret key, endpoints for both buckets
...
repo2-s3-endpoint=<account-id>.r2.cloudflarestorage.com
...
Verify both buckets received segments via R2 dashboard.
...
- [ ] If RTO > 2h: adjust pgbackrest config (`process-max`, `archive-async`, network bandwidth to R2)
...
- Backup encryption at rest beyond R2's default (R2 encrypts by default; pgbackrest can layer with `repo-cipher-pass` if needed; defer until compliance requires it)
...
2026-06-06 alongside other Track B plans. Operational rollout pending operator availability + R2 account access.
