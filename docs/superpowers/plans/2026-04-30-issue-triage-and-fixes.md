...
**Tech Stack:** Go 1.24, GStreamer pipelines, gRPC, PostgreSQL/GORM, `gh` CLI, SSH to production at `<ssh-user>@<v1-prod-host>`.
...
Run from the server (`ssh <ssh-user>@<v1-prod-host>`), inside `/srv/docker/grimnir_radio`:
