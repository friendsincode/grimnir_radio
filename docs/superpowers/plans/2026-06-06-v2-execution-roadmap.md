...
1. **Phase 0 — operational substrate.** Redis, MinIO, ntfy.sh, pgbackrest. Without these, no Track B chunk can deploy cleanly. Skipping any of these = tech debt that compounds when later chunks try to use them.
...
5. **Phase 4 — A6 fan-out + A2 MinIO media migration.** Inbound live audio + media library on MinIO.
...
| MinIO buckets for media + backups | ⏳ pending — **operator action: stand up the MinIO VM (`192.168.195.24`) + buckets** | MinIO VM |
| ntfy.sh self-hosted VPS | ⏳ pending — **operator action: provision a small VPS separate from grimnir hosts** | TBD |
| Media library migrated to MinIO | ⏳ pending — waits on MinIO bucket |
| pgbackrest swapped from POSIX repo to MinIO | ⏳ pending — waits on MinIO bucket (3 config lines once MinIO exists) |

**Operator-pending list** (no engineering work, just account / VPS provisioning + the operator running an rclone migration once):

1. **MinIO** — stand up the MinIO VM (`192.168.195.24`); create buckets `grimnir-backup-lab` + `grimnir-backup-lab-dr` (the `-dr` offsite copy deferred until a second MinIO exists); create an access key scoped to those two; record endpoint / key / secret somewhere durable (Vault when B-4 lands; for now an env file with 0600 perms).
...
3. **Media migration** — once MinIO exists: `rclone sync /srv/data/grimnir_radio/media-data minio:grimnir-media-lab --progress` from current prod.
4. **pgbackrest MinIO swap** — update `/etc/pgbackrest/pgbackrest.conf` to point `repo1-type=s3` at the MinIO bucket; one stanza-upgrade; verify.
...
## Phase 4 — A6 fan-out + A2 MinIO media migration

**A6 fan-out plan:** `docs/superpowers/plans/2026-06-05-live-input-fan-out.md` (521 lines, 12 chunks)
**A2 MinIO migration:** subsumed into Phase 0's operator-pending list + `docs/superpowers/plans/2026-06-05-object-storage-decision.md`

A6 is 5-7 weeks. A2 is operational, completes in Phase 0 once MinIO exists.
...
| Phase | Status | Effort |
|---|---|---|
| Phase 0 substrate | partial: Postgres+Redis+pgbackrest done; MinIO+ntfy+migration pending operator | 1 operator afternoon |
